...
8. [X] 입력값을 검증한다.
   1. [X] ", "를 기준으로 3개로 구분되는 지 검증한다.
   2. [X] 각각 과정, 레벨, 미션에 해당하는 지 검증한다.

9. [X] 파일을 읽어들여 크루를 생성한다.

10. [X] 페어 매칭한다.
    1. [X] 크루가 1명 이하일 경우 에러를 발생시킨다.
    2. [X] 같은 레벨의 페어 목록을 확인한다.
    3. [X] 크루를 섞어 둘씩 짝짓는다. (3명만 남은 경우 셋을 짝짓는다.)
    4. [X] 이미 존재하는 페어라면 다시 섞는다.
    5. [X] 세번 이상 실패하면 에러를 발생시킨다.

11. [X] 이미 매칭된 페어가 있을 경우 재매칭 여부를 입력받는다.

12. [X] 입력값을 검증한다.
    1. [X] "네", "아니오" 중 하나인 지 검증한다.
...
[출처](https://github.com/woowacourse/java-pairmatching-precourse)
